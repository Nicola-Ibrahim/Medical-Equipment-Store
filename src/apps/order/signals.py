...
def decrement_quantity(sender, instance, created, **kwargs):
    ...
    if new_quantity < 0:
        raise ValidationError(
            f"You exceed the number of available for the {instance.product}:{instance.product.quantity}"
        )
...
def increment_quantity(sender, instance, **kwargs):
    ...
    assert not instance.status == Order.Status.DELIVERED, ValidationError(
        "The order is delivered and can't be decrement"
    )
